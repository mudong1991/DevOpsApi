...
from . import utils
from .custom_dulwich_client import get_transport_and_path
...
class Gittle(object):
    ...
    def pull_from(self, origin_uri, branch_name=None):
        return self.fetch(origin_uri, branch_name)
    ...
    def fetch(self, origin_uri=None, branch_name=None, bare=None, origin=None, ):
        bare = bare or False
        origin = origin or self.DEFAULT_REMOTE
        branch = branch_name or self.DEFAULT_BRANCH
        ...
        try:
            commit_sha_branch = self.refs[self._format_ref_branch(branch)]
        except Exception:
            raise Exception('The %s branch was not found' % str(branch))

        # Checkout working directories
        if not bare and self.has_commits:
            self.checkout_all(commit_sha=commit_sha_branch)
        else:
            self.update_server_info()

    @classmethod
    def clone(cls, origin_uri, local_path, auth=None, branch_name=None, mkdir=True, bare=False, *args, **kwargs):
        ...
        repo.fetch(branch_name=branch_name, bare=bare)
